...
//忽略表
export const ignoreTable=(text,ignoreFanTi=false,ignoreYiTi=false,ignoreCustom=false)=>{
    //记录原来的文字列表
    let textArr=[]
...
export const getDiffText=(text1,text2)=>{
    ...
    for (let i=1;i<diffList.length+1;i++){
        ...
        if(i<diffList.length&&i>0&&diffList[i-1].removed===true&&diffList[i].added===true){
            ...
            let flag=new Date().getTime().toString()+Math.round(Math.random()*1000).toString()
    ...
    text1List.forEach((obj)=>{
        for (let char of obj.value){
            res1.push({'value':char,'diff':obj.diff,'added':obj.added,'removed':obj.removed,'order':obj.order})
        }
    })
    text2List.forEach((obj)=>{
        for (let char of obj.value){
            res2.push({'value':char,'diff':obj.diff,'added':obj.added,'removed':obj.removed,'order':obj.order})
        }
    })
...
export const addHighLightByAttribute=(obj)=>{
    if(obj.type==='diff'){
        ...
        return '<span class="text-diff">'+obj.value+'</span>';
...
//处理文本高亮
export const highlightHandler=(text1,text2,ignoreSign=true,ignoreFanTi=false,ignoreYiTi=false,ignoreCustom=false)=>{
    //0.忽略符号
    let text1SignObj={}
    let text2SignObj={}
    if(ignoreSign){
    ...
    //0.忽略表
    let ignoreTableRes1=ignoreTable(text1,ignoreFanTi,ignoreYiTi,ignoreCustom)
    let ignoreTableRes2=ignoreTable(text2,ignoreFanTi,ignoreYiTi,ignoreCustom)//originTextArr newText
